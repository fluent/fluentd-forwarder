...
const ImportPathBase = "github.com/treasure-data/fluentd-forwarder"
...
func main() {
	...
	for _, app := range os.Args[1:] {
		cmd := exec.Command("go", "get", "-ldflags", fmt.Sprintf("-X %s %s", VersionStringVarName, rev), path.Join(ImportPathBase, "entrypoints", app))
